...
## Part1
This part includes solution code from problem 1 (two sum) to 4 (Median of Two Sorted Arrays).
...
For problem 4, the request is an algorithm with time complexity of O(log(m+n)), so binary search algorithms are needed here. One trick to make the code quicker is to determine the size first and by recalling the function to simply swap the position of nums1 and nums2.
